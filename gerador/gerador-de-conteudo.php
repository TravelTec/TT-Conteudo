    ...
    function import_content(){ 
		...
		if(intval(get_option( 'valueInfo' )) == 89){
			...
			for($i=0; $i<count($retornoPosts); $i++){ 
				...
				$id_featured_image = (empty($retornoPostThumbnail["meta_value"]) || is_null($retornoPostThumbnail) ? '' : $retornoPostThumbnail["meta_value"]); 




				if(!empty($id_featured_image)){
			...
			/* CATEGORIAS */
				...
				$sqlDeleteCatBySlug = $connSite->prepare("DELETE FROM ".$table_prefix."terms WHERE slug IN (".substr($deleteCategorySlug, 0, -1).")");
				...
				$sqlDeleteTaxCatByid = $connSite->prepare("DELETE FROM ".$table_prefix."term_taxonomy WHERE term_id IN (".substr($deleteCategoryIdExistente, 0, -1).")");
				...
				$sqlInsertCat = $connSite->prepare("INSERT INTO ".$table_prefix."terms (term_id, name, slug, term_group) VALUES ".substr($insertTags, 0, -2));
				...
				$sqlInsertTaxCat = $connSite->prepare("INSERT INTO ".$table_prefix."term_taxonomy (term_id, taxonomy, description, parent, count) VALUES ".substr($insertTagTaxonomy, 0, -2));
			...
			/* TAGS */ 
				...
				$sqlDeleteTagBySlug = $connSite->prepare("DELETE FROM ".$table_prefix."terms WHERE term_id IN (".substr($deleteTagIdExistente, 0, -1).")");
				...
				$sqlDeleteTaxTagByid = $connSite->prepare("DELETE FROM ".$table_prefix."term_taxonomy WHERE term_id IN (".substr($deleteTagIdExistente, 0, -1).")");
				...
				$sqlInsertTag = $connSite->prepare("INSERT INTO ".$table_prefix."terms (term_id, name, slug, term_group) VALUES ".substr($insertTags, 0, -2));
				...
				$sqlInsertTaxTag = $connSite->prepare("INSERT INTO ".$table_prefix."term_taxonomy (term_id, taxonomy, description, parent, count) VALUES ".substr($insertTagTaxonomy, 0, -2));
			...
			/* POSTS */ 
				...
				if(!empty($resposta["posts_array"]) || !is_null($resposta["posts_array"])){
					...
					for($count=0; $count<count($resposta["posts_array"]); $count++){
						...
						if(!empty($setCategoryPost[$postID])){
							...
							if(count(array_intersect($setCategoryPost[$postID], $getCategories)) > 0){
								...
								$sqlDeletePostByName = $connSite->prepare("DELETE FROM ".$table_prefix."posts WHERE post_name = '".$postName."'");
								...
								$sqlInsertPost = $connSite->prepare("INSERT INTO ".$table_prefix."posts (post_author, post_date, post_date_gmt, post_content, post_title, post_excerpt, post_status, post_name, post_modified, post_modified_gmt, post_type) VALUES ".$dados_posts_json);
								...
								if($filepath != "null" || $filepath != null){
									...
									if($retornoFeaturedImage == false){ 
									    ...
									    $sqlInsertFeaturedImage = $connSite->prepare("INSERT INTO ".$table_prefix."posts (post_author, post_title, post_name, post_date, post_date_gmt, post_modified, post_modified_gmt, post_type, guid, post_status, post_mime_type, post_parent) VALUES (2, '".$postTitle."', '".$nameImageBD."', '".$date."', '".$date."', '".$date."', '".$date."', 'attachment', '".$image["url"]."', 'inherit', '".$image["type"]."', '".$post_id."')");
									...
									$sqlInsertMeta2FeaturedImage = $connSite->prepare("INSERT INTO ".$table_prefix."postmeta (meta_value, meta_key, post_id) VALUES ('".$nameImageBD."', '_wp_attached_file', '".$post_id."')"); 
								...
								if(!is_null($setMetaRankPost[$postID])){
									...
									for($t=0; $t<count($setMetaRankPost[$postID]); $t++){   
										...
										$sqlInsertRankMeta = $connSite->prepare("INSERT INTO ".$table_prefix."postmeta (meta_key, meta_value, post_id) VALUES ('".$setMetaRankPost[$postID][$t][0]."', '".$setMetaRankPost[$postID][$t][1]."', ".$post_id.")");
								...
								if(!is_null($setCategoryPost[$postID])){
									...
									$sqlCategoryTaxonomyRelationship = $connSite->prepare("SELECT * FROM `".$table_prefix."term_relationships` WHERE object_id = ".$post_id);
									...
									for($x=0; $x<count($setCategoryPost[$postID]); $x++){  
										...
										$sqlCategoryTaxonomy = $connSite->prepare("SELECT term_taxonomy_id FROM `".$table_prefix."term_taxonomy` WHERE term_id = ".$setCategoryPost[$postID][$x]);
										...
										if(!empty($retornoCategoryTaxonomy['term_taxonomy_id'])){
											$queryCategory = "INSERT INTO ".$table_prefix."term_relationships (object_id, term_taxonomy_id) VALUES (".$post_id.", ".$retornoCategoryTaxonomy['term_taxonomy_id'].")";

											$sqlInsertCategoryRelationship = $connSite->prepare($queryCategory);



										   $sqlInsertCategoryRelationship->execute();  
										}
								...
								if(!is_null($setTagPost[$postID])){
									...
									for($y=0; $y<count($setTagPost[$postID]); $y++){    
										...
										$sqlTagTaxonomy = $connSite->prepare("SELECT * FROM `".$table_prefix."terms` WHERE slug = '".$setTagPost[$postID][$y][1]."'");
										...
										if(!is_null($retornoTagTaxonomy) || !empty($retornoTagTaxonomy)){
											...
											$sqlTermTagTaxonomy = $connSite->prepare("SELECT term_taxonomy_id FROM `".$table_prefix."term_taxonomy` WHERE term_id = ".$term_id);
										...
										if(!empty($retornoTermTagTaxonomy['term_taxonomy_id'])){

											$queryTag = "INSERT INTO ".$table_prefix."term_relationships (object_id, term_taxonomy_id) VALUES (".$post_id.", ".$retornoTermTagTaxonomy['term_taxonomy_id'].")";

											$sqlInsertTagRelationship = $connSite->prepare($queryTag);



										    $sqlInsertTagRelationship->execute();   
										}
