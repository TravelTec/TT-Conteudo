<?php
    ...
    add_action( 'wp_ajax_import_content_daily', 'import_content_daily' );
    add_action( 'wp_ajax_nopriv_import_content_daily', 'import_content_daily' );
    function import_content_daily(){ 
		...
		$sqlPosts = $connBlog->prepare("SELECT ID, post_author, post_date, post_date_gmt, post_content, post_title, post_excerpt, post_status, post_name, post_modified, post_modified_gmt, post_type FROM `wp_posts` where post_date > '".$dateAfter."' and post_date < '".$dateBefore."'");
	    $sqlPosts->execute(); 
		$retornoPosts = $sqlPosts->fetch(\PDO::FETCH_ASSOC);  

	    $postId = $retornoPosts["ID"];

	    $resposta = [];

		$resposta["posts"] = '('.$retornoPosts["post_author"].', \''.$retornoPosts["post_date"].'\', \''.$retornoPosts["post_date_gmt"].'\', \''.$retornoPosts["post_content"].'\', \''.$retornoPosts["post_title"].'\', \''.$retornoPosts["post_excerpt"].'\', \''.$retornoPosts["post_status"].'\', \''.$retornoPosts["post_name"].'\', \''.$retornoPosts["post_modified"].'\', \''.$retornoPosts["post_modified_gmt"].'\', \''.$retornoPosts["post_type"].'\')';
		$resposta["posts_array"] = array("post_author" => $retornoPosts["post_author"], "post_date" => $retornoPosts["post_date"], "post_date_gmt" => $retornoPosts["post_date_gmt"], "post_content" => $retornoPosts["post_content"], "post_title" => $retornoPosts["post_title"], "post_excerpt" => $retornoPosts["post_excerpt"], "post_status" => $retornoPosts["post_status"], "post_name" => $retornoPosts["post_name"], "post_modified" => $retornoPosts["post_modified"], "post_modified_gmt" => $retornoPosts["post_modified_gmt"], "post_type" => $retornoPosts["post_type"]); 

		/**** *****/ 

		$sqlPostMeta = $connBlog->prepare("SELECT * FROM `wp_postmeta` WHERE post_id = ".$postId);
	    $sqlPostMeta->execute(); 
		$retornoPostMeta = $sqlPostMeta->fetch(\PDO::FETCH_ASSOC);  
		...
		$resposta["featured_image_posts"] = $post_featured_image;
		...
		for($i=0; $i<count($retornoPostCategory); $i++){
			$resposta["categories_posts"][] = $retornoPostCategory[$i]["term_taxonomy_id"];
		}
		...
		for($i=0; $i<count($retornoPostTag); $i++){
			$resposta["tags_posts"][] = array($retornoPostTag[$i]["term_taxonomy_id"], $retornoPostTag[$i]["slug"]);
		}
		...
		*/
		...
		/* CATEGORIAS */
			...
			/* DELETA CATEGORIAS PELO ID */
			$sqlDeleteCatByid = $connSite->prepare("DELETE FROM wp_terms WHERE id IN ".substr($deleteCategoryIdExistente, 0, -1));
		    $sqlDeleteCatByid->execute(); 
			/* */ 

			/* DELETA CATEGORIAS PELO SLUG */ 
			$sqlDeleteCatBySlug = $connSite->prepare("DELETE FROM wp_terms WHERE id IN ".substr($deleteCategorySlug, 0, -1));
			...
			/* INSERE TAXONOMIA */
			$sqlInsertTaxCat = $connSite->prepare("INSERT INTO wp_term_taxonomy (term_id, name, slug, term_group) VALUES ".substr($insertTagTaxonomy, 0, -2));
		...
		/* TAGS */ 
			...
			/* DELETA CATEGORIAS PELO ID */
			$sqlDeleteTagByid = $connSite->prepare("DELETE FROM wp_terms WHERE id IN ".substr($deleteTagIdExistente, 0, -1));
		    $sqlDeleteTagByid->execute(); 
			/* */ 

			/* DELETA CATEGORIAS PELO SLUG */ 
			$sqlDeleteTagBySlug = $connSite->prepare("DELETE FROM wp_terms WHERE id IN ".substr($deleteTagSlug, 0, -1));
			...
			/* INSERE TAXONOMIA */
			$sqlInsertTaxTag = $connSite->prepare("INSERT INTO wp_term_taxonomy (term_id, name, slug, term_group) VALUES ".substr($insertTagTaxonomy, 0, -2));
		...
		/* POSTS */ 
			...
			if(!empty($resposta["posts_array"]) || !is_null($resposta["posts_array"])){
				...
				$bFound = (bool) array_intersect($setCategoryPost, $getCategories);

				if(count(array_intersect($setCategoryPost, $getCategories)) > 0){
 	
 	 				$posts_array["post_content"] = str_replace('https://redeturistica.com.br/formulario-de-cotacao/', 'https://'.$_SERVER["HTTP_HOST"].'/formulario-de-cotacao/', $posts_array["post_content"]); 

					$postTitle = $posts_array["post_title"];
					$postName = $posts_array["post_name"];
 
					$sqlDeletePostByName = $connSite->prepare("DELETE FROM wp_posts WHERE post_name = '".$postName."'");
				    $sqlDeletePostByName->execute();   
					
					$sqlInsertPost = $connSite->prepare("INSERT INTO wp_posts (post_author, post_date, post_date_gmt, post_content, post_title, post_excerpt, post_status, post_name, post_modified, post_modified_gmt, post_type) VALUES ".$posts);
				    $sqlInsertPost->execute();

					$post_id = $connSite->lastInsertId();

					$filepath = $setFeaturedImagePost; //Change this to the actual file path... 
					...
					if(!is_null($setCategoryPost)){
						for($x=0; $x<count($setCategoryPost); $x++){  

							$sqlCategoryTaxonomy = $connSite->prepare("SELECT term_taxonomy_id FROM `wp_term_taxonomy` WHERE term_id = ".$setCategoryPost[$x]);
					...
					if(!is_null($setTagPost)){
						for($x=0; $x<count($setTagPost); $x++){    

							$sqlTagTaxonomy = $connSite->prepare("SELECT * FROM `wp_terms` WHERE slug = '".$setTagPost[$x][1]."'");
