<?php
    ...
    function import_content_daily(){ 
		...
		$sqlPosts = $connBlog->prepare("SELECT ID, post_author, post_date, post_date_gmt, post_content, post_title, post_excerpt, post_status, post_name, post_modified, post_modified_gmt, post_type FROM `wp_posts` where post_status = 'publish' AND post_date > '".$dateAfter."' and post_date < '".$dateBefore."'");
		...
		for ($x=0; $x<count($retornoPostMeta); $x++) {
			...
			$resposta["rank_meta"][] = array($retornoPostMeta[$x]["meta_key"], $retornoPostMeta[$x]["meta_value"]);
		...
		*/
		...
		/* CATEGORIAS */
			...
			$sqlDeleteCatByid = $connSite->prepare("DELETE FROM wp_terms WHERE term_id IN (".substr($deleteCategoryIdExistente, 0, -1).")");
		...
		/* TAGS */ 
			...
			$sqlDeleteTagByid = $connSite->prepare("DELETE FROM wp_terms WHERE term_id IN (".substr($deleteTagIdExistente, 0, -1).")");
		...
		/* POSTS */ 
			...
			if(!empty($resposta["posts_array"]) || !is_null($resposta["posts_array"])){
				...
				if(count(array_intersect($setCategoryPost, $getCategories)) > 0){
					...
					if(!is_null($setMetaRankPost)){

						for($t=0; $t<count($setMetaRankPost); $t++){   

							$sqlInsertRankMeta = $connSite->prepare("INSERT INTO wp_postmeta (meta_key, meta_value, post_id) VALUES ('".$setMetaRankPost[$t][0]."', '".$setMetaRankPost[$t][1]."', ".$post_id.")");
